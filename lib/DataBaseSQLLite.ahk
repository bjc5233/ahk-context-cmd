...
class DataBaseSQLLite extends DBA.DataBase
{
	...
	*/
	...
	Update(record, tableName){
		...
		dbColums := []
		for i,row in this._GetTableObj("PRAGMA table_info(" this.QuoteIdentifier(tableName) ")").Rows {
			dbColums[row.name] := row.type
		}
		
		colString := ""
		updateColums := []
		colIndex := 1
		for colName, value in record {
			if (colName == "id")
				continue
			if (!dbColums.HasKey(colName))
				continue
			
			colString .= "," this.QuoteIdentifier(colName) "=?"
			updateColums[colName] := {"type": dbColums[colName], "index": colIndex}
			colIndex ++
		}
		updateColums["id"] := {"type": dbColums["id"], "index": colIndex}
		...
		try {
			for col, val in record {
				if (!updateColums.HasKey(col))
					continue
				fieldType := updateColums[col]["type"]
				if (!fieldType)
					fieldType := "auto"
				fieldIndex := updateColums[col]["index"]
				ret := SQLite_bind(query, fieldIndex, val, fieldType)
			}
			SQLite_Step(query)
			SQLite_Reset(query)
